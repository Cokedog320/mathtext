# Left Sidebar Accordion Refactoring & Codebase Modularization Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Refactor the math worksheet customizer to extract the left sidebar into an accordion-style component, modularize `App.tsx` into clean components and logic helpers, and eliminate nested `if` statements using Strategy and Config Registry patterns.

**Architecture:** Split types, generator logic, problem renderers, sidebar, and A4 worksheet into separate, decoupled files. Integrate them in `App.tsx` using standard props passing. Use registries and strategies to map modes to generator/layout logic instead of giant `if` checks.

**Tech Stack:** React 19, TypeScript, Tailwind CSS, Lucide Icons, Vitest

## Global Constraints
- Do not introduce runtime bugs or break existing behavior.
- Ensure all 28 existing unit tests continue to pass without modification to their core test assertions.
- Use explicit types and strict TypeScript compiler checks.
- Keep components responsive and follow tailwind visual rules.

---

## File Structure Map
Before modifying code, here is the target file structure:
- Create: [src/types.ts](file:///home/cokedog/mathtext/src/types.ts) — types, translations, file name maps.
- Create: [src/utils/problemGenerator.ts](file:///home/cokedog/mathtext/src/utils/problemGenerator.ts) — problem generator functions and Strategy Registry.
- Create: [src/components/ProblemRenderers.tsx](file:///home/cokedog/mathtext/src/components/ProblemRenderers.tsx) — NumberBond, VerticalArithmetic, HorizontalArithmetic, MethodDiagram.
- Create: [src/components/Sidebar.tsx](file:///home/cokedog/mathtext/src/components/Sidebar.tsx) — Left sidebar with accordion selector and settings inputs.
- Create: [src/components/Worksheet.tsx](file:///home/cokedog/mathtext/src/components/Worksheet.tsx) — Worksheet render sheet, A4 wrapper, layout registry.
- Modify: [src/App.tsx](file:///home/cokedog/mathtext/src/App.tsx) — Orchestrate states and high-level layout.
- Modify: [src/App.test.ts](file:///home/cokedog/mathtext/src/App.test.ts) — Update import statements.

---

### Task 1: Create Types and Translations Constants

**Files:**
- Create: [src/types.ts](file:///home/cokedog/mathtext/src/types.ts)

**Interfaces:**
- Consumes: None
- Produces: Type definitions (`Language`, `Range`, `Mode`, `RegroupOption`, `Problem`), translations, and pdf name mappings.

- [ ] **Step 1: Write `src/types.ts`**
  Write the core types and translations extracted from `App.tsx`.
...
- [ ] **Step 2: Commit Task 1**
  ...
  git commit -m "feat: extract types and translations into src/types.ts"
  ```

---

### Task 2: Strategy-Based Problem Generator Utilities

**Files:**
- Create: [src/utils/problemGenerator.ts](file:///home/cokedog/mathtext/src/utils/problemGenerator.ts)

**Interfaces:**
- Consumes: Types from `src/types.ts`
- Produces: `generateProblems`, `shuffle`, `getPrintTitle`

- [ ] **Step 1: Write `src/utils/problemGenerator.ts`**
  Implement the strategy pattern for problem generation, splitting different mode generation logics into focused helper functions.
...
- [ ] **Step 2: Commit Task 2**
  Run:
  ```bash
  git add src/utils/problemGenerator.ts
  git commit -m "feat: implement strategy pattern for problem generation"
  ```

---

### Task 3: Problem Components Renderers

**Files:**
- Create: [src/components/ProblemRenderers.tsx](file:///home/cokedog/mathtext/src/components/ProblemRenderers.tsx)

**Interfaces:**
- Consumes: Types from `src/types.ts`
- Produces: `NumberBond`, `VerticalArithmetic`, `HorizontalArithmetic`, `MethodDiagram` React components.

- [ ] **Step 1: Write `src/components/ProblemRenderers.tsx`**
  Implement clean rendering components extracted from `App.tsx`.
...
- [ ] **Step 2: Commit Task 3**
  Run:
  ```bash
  git add src/components/ProblemRenderers.tsx
  git commit -m "feat: extract problem rendering components"
  ```
...
**Files:**
- Create: [src/components/Sidebar.tsx](file:///home/cokedog/mathtext/src/components/Sidebar.tsx)
...
- [ ] **Step 1: Write `src/components/Sidebar.tsx`**
  Implement the collapsible Sidebar component with Method Training and Arithmetic Practice accordions.
...
- [ ] **Step 2: Commit Task 4**
  ...
  git commit -m "feat: implement Sidebar component with Category Accordions"
  ```

---

### Task 5: Worksheet Preview and Layout Registry

**Files:**
- Create: [src/components/Worksheet.tsx](file:///home/cokedog/mathtext/src/components/Worksheet.tsx)

**Interfaces:**
- Consumes: Types from `src/types.ts`, Components from `src/components/ProblemRenderers.tsx`, Functions from `src/utils/problemGenerator.ts`.
- Produces: `Worksheet` and `A4PreviewWrapper` components.

- [ ] **Step 1: Write `src/components/Worksheet.tsx`**
  Implement the `Worksheet` renderer using the layout config registry map.
...
interface RenderConfig {
  component: React.FC<{ problem: any; index: number; large?: boolean; hideParts?: boolean; hideTen?: boolean }>;
...
- [ ] **Step 2: Commit Task 5**
  ...
  git commit -m "feat: implement Worksheet component and A4 preview logic"
  ```

---

### Task 6: Orchestrate in App.tsx

**Files:**
- Modify: [src/App.tsx](file:///home/cokedog/mathtext/src/App.tsx)

**Interfaces:**
- Consumes: All components from Tasks 1-5.
- Produces: Integrated clean App wrapper.

- [ ] **Step 1: Overwrite `src/App.tsx`**
  Modify [App.tsx](file:///home/cokedog/mathtext/src/App.tsx) to tie all components together.
...
- [ ] **Step 2: Commit Task 6**
  ...
  ```bash
  git add src/App.tsx
  git commit -m "refactor: clean App.tsx and integrate Sidebar and Worksheet components"
  ```

---

### Task 7: Update Imports in Tests

**Files:**
- Modify: [src/App.test.ts](file:///home/cokedog/mathtext/src/App.test.ts)

**Interfaces:**
- Consumes: `generateProblems` from `src/utils/problemGenerator.ts`
- Produces: Correct test import bindings.

- [ ] **Step 1: Modify imports in `src/App.test.ts`**
  Change the import statement at the top from:
  ```typescript
...
- [ ] **Step 3: Run production build compilation check**
  Run: `npm run build`
  Expected output: Compilation success with zero type errors.

- [ ] **Step 4: Commit Task 7**
  Run:
  ```bash
  git add src/App.test.ts
  git commit -m "test: update imports in App.test.ts and verify passes"
  ```
